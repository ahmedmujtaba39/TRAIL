...
def main() -> None:
    ...
    parser.add_argument("--offset", type=int, default=0, help="Skip this many sorted qualified source clips (for a disjoint held-out split).")
    ...
    parser.add_argument("--window-selection", choices=["random", "motion_change"], default="random", help="Weak-window sampling proxy; motion_change targets high wrist velocity changes.")
    ...
    count = write_windows(poses, args.output, windows_per_clip=args.windows_per_clip, boundary_frames=args.boundary_frames, duration=args.duration, handshape_model=handshape, device=device, selection=args.window_selection)
